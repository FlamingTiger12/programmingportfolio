    ...
    for (int i = 0; i < powerups.size(); i++) {
      ...
      if (powerup.buff(blue)) { //Has the ship hit a powerup?
        if (powerup.type == 'h') { //Health Benefit
          ...
          blue.laserCount += 10;
          text ("+10 Ammo", powerup.x, powerup.y);
          score += 10;
...
void infoPanel() { //Display statistics
  ...
  text("Ammo: " + blue.laserCount, 480, height-20); //Ammo or Laser Count
}
...
void startScreen() {
  ...
  textSize(24);
  text("Katelyn Lin", width/2, 220);
  textSize(24);
  text("Click/Press Space to Start", width/2, 350);
